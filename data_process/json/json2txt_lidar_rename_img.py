...
def get_json(json_file, out_dir, filename):
    ...
    for i in range(len(objects)):
        ...
        # 图像
        print((content["objects"][i])["box2d"])
        if((content["objects"][i])["box2d"]=={} or "x" not in (content["objects"][i])["box2d"]["camera1"] ):
            img_x =  0 # 9
            img_y =  0  # 10
            img_h =  0  # 11
            img_w =  0  # 12
        else:
            img_x =  (content["objects"][i])["box2d"]["camera1"]["x"] # 9
            img_y =  (content["objects"][i])["box2d"]["camera1"]["y"] # 10
            img_h =  (content["objects"][i])["box2d"]["camera1"]["height"] # 11
            img_w =  (content["objects"][i])["box2d"]["camera1"]["width"] # 12
        str_tmp += str(img_x) + " " + str(img_y)+ " " + str(img_h)+ " "+ str(img_w)+ " "  ##暂存内容
        # 
        str_tmp += str(yaw) + " " +  str(trackid) # 行尾 13
        #   换行
        str_tmp +=  "\n"  ##暂存内容
    fp.write(str_tmp)
    fp.close()
